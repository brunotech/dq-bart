...
class BartAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = QuantizeLinear(embed_dim, embed_dim, config=config)
        self.v_proj = QuantizeLinear(embed_dim, embed_dim, config=config)
        self.q_proj = QuantizeLinear(embed_dim, embed_dim, config=config)
        self.out_proj = QuantizeLinear(embed_dim, embed_dim, config=config)
...
class BartEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.fc1 = QuantizeLinear(self.embed_dim, config.encoder_ffn_dim, config=config)
        self.fc2 = QuantizeLinear(config.encoder_ffn_dim, self.embed_dim, config=config)
...
class BartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.fc1 = QuantizeLinear(self.embed_dim, config.decoder_ffn_dim, config=config)
        self.fc2 = QuantizeLinear(config.decoder_ffn_dim, self.embed_dim, config=config)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
...
class BartPretrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (nn.Embedding, QuantizeEmbedding)):
            module.weight.data.normal_(mean=0.0, std=std)
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if embed_tokens is not None:
            ...
            self.embed_tokens = QuantizeEmbedding(config.vocab_size, embed_dim, padding_idx=self.padding_idx, config=config)
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        ...
        self.shared = QuantizeEmbedding(vocab_size, config.d_model, padding_idx=padding_idx, config=config)
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def resize_token_embeddings(self, new_num_tokens: int) -> QuantizeEmbedding:  #nn.Embedding:
    ...
    def forward(
        ...
        if labels is not None:
            if self.config.problem_type is None:
                ...
                elif self.config.num_labels > 1 and labels.dtype in [
                    torch.long,
                    torch.int,
                ]:
